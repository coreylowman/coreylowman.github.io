...
```rust
// a 2d tensor with shape of 2x3 known at compile time
let _: Tensor<(2, 3)> = ...
// a 2d tensor with shape known at runtime
let _: Tensor<(usize, usize)> = ...
// a 3d tensor of shape with batch dimension known at runtime
let _: Tensor<(usize, 4, 5)> = ...
// a 4d tensor with 2 runtime dimensions and 2 compile time dimensions
...
Now for runtime dimensions, we can just use `usize`:
...
let _: Tensor<(usize, usize)> = ...
let _: Tensor<(usize, Const<4>, Const<5>)> = ...
let _: Tensor<(Const<1>, usize, Const<2>, usize)> = ...
```
